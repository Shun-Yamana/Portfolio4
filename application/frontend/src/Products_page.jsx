...
function ProductsPage({ initialData, onChoose }) {
  ...
  const handleRandomChoose = () => {
    ...
    const randomIndex = Math.floor(Math.random() * interestedProducts.length)
    const randomProduct = interestedProducts[randomIndex]
    handleChoose(randomProduct)
  }
  ...
  return (
    <div className="products-page">
      ...
      {interestedProducts.length > 0 && (
        <div className="products-page__random-wrap">
          <button
            type="button"
            className="product-card__button product-card__button--random"
            onClick={handleRandomChoose}
            disabled={loading}
          >
            🎲 もう任せた！
          </button>
      ...
      {interestedProducts.length > 0 && (
        <section className="interested-section">
          <h2 className="interested-section__title">気になると押したもの</h2>
          <div className="products-grid">
            {interestedProducts.map((product) => (
              <ProductsCard
                key={`interested-${product.id}`}
                name={product.name}
                price={product.price}
                description={product.description}
                image={productImages[product.image] || custardChouxImage}
